    ...
    <main className="flex flex-col justify-center">
      ...
      <section className="flex flex-col gap-4">
        {users?.map((user, i) => (
          <div
            key={i}
            className="bg-zinc-800 rounded-2xl flex items-center justify-baseline px-4 py-2 gap-4"
            onClick={() => data(user)}
          >
            <div className="h-12 w-12 relative rounded-full bg-linear-to-tr from-yellow-400 via-pink-500 to-purple-600">
              <Image
                src={
                  user?.profilePicture ||
                  "https://plus.unsplash.com/premium_photo-1739580360043-f2c498c1d861?w=600"
                }
                alt={user.fullName}
                fill
                className="object-cover rounded-full h-full w-full p-1"
              />
            </div>
