    ...
    <main className="min-h-screen bg-black text-white w-full overflow-y-auto pb-20">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <header className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-16">
          <div className="relative h-28 w-28 md:h-36 md:w-36 rounded-full border-2 border-zinc-800 p-1 shrink-0">
          ...
          <div className="flex flex-col items-center md:items-start gap-4 flex-1 w-full">
            <div className="flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="flex gap-2">
                <Link href="/edit-profile" className="bg-white/10 hover:bg-white/20 px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2">
                  <FiEdit2 /> Edit Profile
                </Link>
                <button className="bg-white/10 hover:bg-white/20 px-4 py-1.5 rounded-lg text-sm font-semibold transition-colors flex items-center gap-2">
            ...
            <div className="text-sm md:text-base text-center md:text-left mt-2 md:mt-0">
              ...
              <p className="text-zinc-400 mt-1">{userInfo?.bio}</p>
            </div>
          </div>
        </header>

        <nav className="flex justify-center border-t border-zinc-800 mt-12 mb-2 gap-12 text-xs md:text-sm text-zinc-400 font-semibold tracking-widest uppercase">
          {[
...
function PostsGrid({ data, isOwnPost }: { data: PostProps[]; isOwnPost?: boolean }) {
  ...
  if (!data?.length) return <div className="text-center text-zinc-500 py-16 font-medium">No Posts Yet</div>;

  return (
    <>
      <div className="grid grid-cols-3 gap-1 md:gap-4">
        {data.map((post) => (
          <div key={post._id} onClick={() => setShowPost(post)} className="relative aspect-square cursor-pointer group bg-black overflow-hidden md:rounded-md border border-zinc-800">
...
function PostModal({ post, onClose, isOwnPost }: { post: PostProps; onClose: () => void; isOwnPost?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
      {showConfirm ? (
        <div className="bg-black border border-zinc-800 p-8 rounded-2xl max-w-sm w-full text-center shadow-2xl">
        ...
        <div className="relative w-full max-w-5xl h-[85vh] flex flex-col md:flex-row bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl">
          ...
          <div className="flex-1 relative bg-black flex items-center justify-center border-b md:border-b-0 md:border-r border-zinc-800 min-h-[40vh] md:min-h-0">
            {isVideo ? <video src={url} controls autoPlay className="max-h-full max-w-full" /> : <Image src={url} alt="" fill className="object-contain" />}
          </div>
          <div className="w-full md:w-[350px] lg:w-[400px] flex flex-col bg-black h-[50vh] md:h-full">
            <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
              ...
              <button onClick={onClose} className="hidden md:block p-2 text-zinc-400 hover:text-white hover:bg-white/10 rounded-full transition-colors"><FiX size={22} /></button>
            </div>
            <div className="flex-1 p-4 overflow-y-auto"></div>
            {canModify && (
              <div className="p-4 border-t border-zinc-800 flex gap-3">
                <button onClick={() => setShowConfirm(true)} className="flex-1 flex justify-center items-center gap-2 py-2.5 bg-black border border-zinc-800 hover:border-red-500/50 hover:bg-red-500/10 text-red-500 rounded-xl text-sm font-bold transition-all"><FiTrash2 /> Delete</button>
